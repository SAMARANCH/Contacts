...
public class BootCmpReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(TelephonyIntents.ACTION_PHB_STATE_CHANGED)) {
			//BUG #4382,delete these lines for sometimes sim card contacts can't show after boot,chengting,@20150507,begin
			/*
			...
			*/
        	//BUG #4382,delete these lines for sometimes sim card contacts can't show after boot,chengting,@20150507,end
			...
			*/
			/// bird: BUG #6213,ALPS02233935,chengting,@20150806
	...
	private boolean isPhbReady() {
		...
		for (int slotId = 0; slotId < simCount; slotId++) {
			...
			if (SimCardUtils.isPhoneBookReady(subId[0])) {
				return true;
			}
		}
		return false;
