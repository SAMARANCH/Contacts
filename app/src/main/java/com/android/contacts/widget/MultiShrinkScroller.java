...
public class MultiShrinkScroller extends FrameLayout {
    ...
    ///M:[for rcs-e]@{
    private ImageView mJoynIconView;
    ///@}
    ///M:[for rcs]@{
    private ImageView mRcsIconView;
    ///@}
    ...
    ///M:[for rcs-e]@{
    /**
     * create Joyn icon view shown on the letp top
     */
    private void createJoynIconView() {
        FrameLayout container = (FrameLayout) findViewById(R.id.placeholder_container);
        Uri lookupUri = ((Activity) getContext()).getIntent().getData();
        mJoynIconView = (ImageView) ExtensionManager.getInstance().getViewCustomExtension()
                .getQuickContactScrollerCustom()
                .createJoynIconView(container, mLargeTextView, lookupUri);
    }

    /**
     * update the mJoynIconView Visibility.
     * When to visible
     */
    private void updateJoynView() {
     ...
     * create Joyn icon view shown on the letp top.
     */
    private void createRcsIconView() {
        FrameLayout container = (FrameLayout) findViewById(R.id.placeholder_container);
        Uri lookupUri = ((Activity) getContext()).getIntent().getData();
        mRcsIconView = (ImageView) ExtensionManager.getInstance().getRcsExtension()
                .getQuickContactRcsScroller()
                .createRcsIconView(container, mLargeTextView, lookupUri);
    }
     ...
     * When to visible
     */
    private void updateRcsView() {
        if (mRcsIconView != null) {
