...
public class MultiChoiceHandlerListener {
    ...
    synchronized void onFinished(final int requestType, final int jobId, final int total) {
        ...
        //BUG #4308,chengting,@20150424,begin
        final Intent intent;
        if(BirdCommonFeature.BIRD_DIALER_FTO){
        	intent = new Intent();
        	intent.setClassName("com.android.dialer", "com.bird.contacts.BirdPeopleActivity");
        }else{
        	intent = new Intent(mContext, PeopleActivity.class);
        }
        //BUG #4308,chengting,@20150424,end
