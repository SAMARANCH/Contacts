...
//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,begin
import android.app.Instrumentation;
import com.bird.direct.*;
import android.telephony.SubscriptionManager;
import com.android.internal.telephony.ISub;
import android.os.ServiceManager;
import android.os.RemoteException;
//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,end
//bird
...
public class QuickContactActivity extends ContactsActivity {
    ...
    final OnClickListener mEntryClickHandler = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            //fto contact,add no contacts card,chengting,@20150401,begin
            if(dataId == CARD_ENTRY_ID_ADD_AS_NEW_CONTACT){
            	addAsNewContact(mFirstPhoneNumber);
            	return;
            }
            if(dataId == CARD_ENTRY_ID_ADD_TO_EXIST_CONTACT){
            	addToExistContact(mFirstPhoneNumber);
            	return;
            }
            //fto contact,add no contacts card,chengting,@20150401,end
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //fto,chengting,@20150401,begin
        if(BirdFeature.BIRD_DIALER_FTO){
        	mIconVoiceCall=R.drawable.bird_fto_ic_phone_24dp;
        	mIconSipCall=R.drawable.bird_fto_ic_dialer_sip_black_24dp;
        	mIconMmsSend=R.drawable.bird_fto_ic_message_24dp;
        }
        //fto,chengting,@20150401,end
        ...
        Trace.endSection();
		//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,begin
        int[] totalSubIds = null;
        ISub iSub = null;
        try {
            iSub = ISub.Stub.asInterface(ServiceManager.getService("isub"));
            if (iSub != null) {
                totalSubIds = iSub.getActiveSubIdList();
            }
        } catch (RemoteException ex) {
            // ignore it
        }
        //long[] totalSubIds = SubscriptionManager.getActiveSubIdList();
        if(totalSubIds!=null){
            mInsertedSimCount=totalSubIds.length;
        }
        if(DirectUtil.support(this,mApproachHelper.getTag())){
        	mDirectApproach=new DirectByApproach(this, mApproachHelper);
        }
        //[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,end
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_CONTACT_EDITOR_ACTIVITY &&
            ...
            processIntent(data);
		//fto contact,add no contacts card,chengting,@20150401,begin
        }else if(requestCode == REQUEST_CODE_CONTACT_SAVE_ACTIVITY && resultCode != RESULT_CANCELED){
        	processIntent(data);
		//fto contact,add no contacts card,chengting,@20150401,end
        }
    }
    ...
    private void startInteractionLoaders(Cp2DataCardModel cp2DataCardModel) {
        ...
        if (phoneDataItems != null) {
			...
			//fto contact,add no contacts card,chengting,@20150401,begin
//            if(BirdFeature.BIRD_DIALER_FTO){//BUG #4278,chengting,@20150511
        	if(phoneDataItems.size()>0){
        		mFirstPhoneNumber=phoneNumbers[0];
        	}
//            }
            //fto contact,add no contacts card,chengting,@20150401,end
			//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,begin
            if(DirectUtil.support(this,mApproachHelper.getTag())){
				boolean hasPhone=PhoneCapabilityTester.isPhone(this);
				if(phoneDataItems.size()>0 && hasPhone && mInsertedSimCount>0){
					mPrimiryPhoneIntent = CallUtil.getCallIntent(phoneNumbers[0]);
				}else{
					DirectLog.e(mApproachHelper.getTag(),"hasPhone="+hasPhone
							+ ",phoneDataItems.size()="+phoneDataItems.size()+",mInsertedSimCount="+mInsertedSimCount);
					mPrimiryPhoneIntent = null;
				}
			}
			//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,end
        }
    ...
    protected void onResume() {
		...
		//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,begin
		if(mDirectApproach!=null){
			mDirectApproach.start();
		}
		//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,end
    }
    ...
    private void populateContactAndAboutCard(Cp2DataCardModel cp2DataCardModel) {
		...
		//fto contact,add no contacts card,chengting,@20150401,begin
        if(showSaveCard()){
        	initializeAddContactCard();
        }else if (contactCardEntries.size() == 0 && aboutCardEntries.size() == 0) {
		//fto contact,add no contacts card,chengting,@20150401,end
    ...
    private void initializeNoContactDetailCard() {
        final Drawable phoneIcon = getResources().getDrawable(
                //fto contact,chengting,@20150401
                mIconVoiceCall/*R.drawable.ic_phone_24dp*/).mutate();
        final Entry phonePromptEntry = new Entry(CARD_ENTRY_ID_EDIT_CONTACT,
                ...
                //fto contact,chengting,@20150401
                /* thirdContentDescription = */ null, mIconVoiceCall/*R.drawable.ic_phone_24dp*/);
	...
	///M: Fix ALPS01995031
        ...
        if (dataItem instanceof ImDataItem) {
            ...
            if (!TextUtils.isEmpty(phone.getNumber())) {
                ...
                //fto,chengting,@20150401,begin
                icon = res.getDrawable(mIconVoiceCall/*R.drawable.ic_phone_24dp*/);
                iconResourceId = mIconVoiceCall/*R.drawable.ic_phone_24dp*/;
                //fto,chengting,@20150401,end
                ...
                if (PhoneCapabilityTester.isSupportSms(context)) {
                    ...
                    //fto,chengting,@20150401,begin
                    alternateIcon = res.getDrawable(mIconMmsSend/*R.drawable.ic_message_24dp*/);
                    //fto,chengting,@20150401,end
            ...
            if (!TextUtils.isEmpty(address)) {
                ...
                primaryContentDescription.append(header);
                    //fto,chengting,@20150401,begin
                    icon = res.getDrawable(mIconSipCall/*R.drawable.ic_dialer_sip_black_24dp*/);
                    iconResourceId = mIconSipCall/*R.drawable.ic_dialer_sip_black_24dp*/;
                    //fto,chengting,@20150401,end
            }
    ...
    private void extractAndApplyTintFromPhotoViewAsynchronously() {
		...
		Log.d(TAG, "extractAndApplyTintFromPhotoViewAsynchronously onPostExecute start!");
                ...
                /** @} */
                Log.d(TAG, "extractAndApplyTintFromPhotoViewAsynchronously onPostExecute RCS!");
    ...
    private void setThemeColor(MaterialPalette palette) {
		...
		//fto contact,add no contacts card,chengting,@20150401,begin
        if(showSaveCard()){
        	mNoContactDetailsCard.setColorAndFilter(primaryColor, mColorFilter);
        }
		//fto contact,add no contacts card,chengting,@20150401,end
        /// M: [for RCS-e]
    ...
    private boolean isContactEditable() {
        return mContactData != null && !mContactData.isDirectoryEntry() &&
            !mContactData.isSdnContacts() && !mContactData.isReadOnlyContact();//for read only contacts,chengting,@20141222
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (mContactData != null) {
            ...
            if (ContactsSystemProperties.MTK_HOTKNOT_SUPPORT
                ...
                if(mContactData.isSdnContacts() || !isContactEditable()) {//for read only contacts,chengting,@20141222
	...
	//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,begin
	int mInsertedSimCount=0;
	Intent mPrimiryPhoneIntent;
    DirectByApproach mDirectApproach;
    DirectUtil.DirectApproachHelper mApproachHelper=new DirectUtil.DirectApproachHelper() {
		
		@Override
		public String getTag(){
			return DirectUtil.TAG_CONTACT_CALL;
		}
		
		@Override
		public void approach() {
			DirectLog.d(getTag(),"call intent:"+mPrimiryPhoneIntent);
			if(mPrimiryPhoneIntent!=null){
				startActivity(mPrimiryPhoneIntent);
			}
		}
	};
	//[BIRD_DIRECT_WITH_PROXIMITY][BIRD_DIRECT_CONTACT_CALL],20150119,chengting,end
    ...
    //fto contact,add no contacts card,chengting,@20150401,begin
	protected static int mIconVoiceCall=R.drawable.ic_phone_24dp;
	protected static int mIconSipCall=R.drawable.ic_dialer_sip_black_24dp;
	protected static int mIconMmsSend=R.drawable.ic_message_24dp;
	
    private static final int REQUEST_CODE_CONTACT_SAVE_ACTIVITY = 100;
    private static final int CARD_ENTRY_ID_ADD_AS_NEW_CONTACT = -100;
    private static final int CARD_ENTRY_ID_ADD_TO_EXIST_CONTACT = -101;
    private String mFirstPhoneNumber="";
    private boolean showSaveCard(){
    	if(mContactData!=null){
    		final String lookupKey = mContactData.getLookupKey();
    		Log.d(TAG,"showSaveCard,lookupKey="+lookupKey);
    		if(lookupKey==null){
    			return true;
    		}
    	}
    ...
    private Intent getAddAsNewContactIntent(String number) {
        Intent intent = new Intent(Intent.ACTION_INSERT, Contacts.CONTENT_URI);
        intent.putExtra(ContactEditorActivity.INTENT_KEY_FINISH_ACTIVITY_ON_SAVE_COMPLETED, true);
        intent.putExtra(Insert.PHONE, number);
        return intent;
    }
    private Intent getAddToExistContactIntent(String number) {
    ...
    public void addAsNewContact(String number){
    	mHasIntentLaunched = true;
    	mContactLoader.cacheResult();
        startActivityForResult(getAddAsNewContactIntent(number),REQUEST_CODE_CONTACT_SAVE_ACTIVITY);
        
    }
    public void addToExistContact(String number){
    	mHasIntentLaunched = true;
    	mContactLoader.cacheResult();
    	startActivityForResult(getAddToExistContactIntent(number),REQUEST_CODE_CONTACT_SAVE_ACTIVITY);
    }
	
    private void initializeAddContactCard(){
        mNoContactDetailsCard.setTitle(getResources().getString(R.string.bird_contact_add));
    	Log.d(TAG,"mFirstPhoneNumber="+mFirstPhoneNumber);
        final Drawable saveIcon = getResources().getDrawable(R.drawable.ic_add_to_circles_black_24).mutate();
        final Entry addAsNewEntry = new Entry(CARD_ENTRY_ID_ADD_AS_NEW_CONTACT,
        		saveIcon, getString(R.string.bird_contact_add_as_new),
        		/* subHeader = */ null, /* subHeaderIcon = */ null, /* text = */ null, 
        		/* textIcon = */ null, /* primaryContentDescription = */ null,
                getAddAsNewContactIntent(mFirstPhoneNumber),
                /* alternateIcon = */ null, /* alternateIntent = */ null,
                /* alternateContentDescription = */ null, /* shouldApplyColor = */ true,
                /* isEditable = */ false, /* EntryContextMenuInfo = */ null,
                /* thirdIcon = */ null, /* thirdIntent = */ null,
                /* thirdContentDescription = */ null, R.drawable.ic_add_to_circles_black_24);
        final Entry addToExistEntry = new Entry(CARD_ENTRY_ID_ADD_TO_EXIST_CONTACT,
        		saveIcon, getString(R.string.bird_contact_add_to_exist),
        		/* subHeader = */ null, /* subHeaderIcon = */ null, /* text = */ null, 
        		/* textIcon = */ null, /* primaryContentDescription = */ null,
        		getAddToExistContactIntent(mFirstPhoneNumber),
        		/* alternateIcon = */ null, /* alternateIntent = */ null,
        		/* alternateContentDescription = */ null, /* shouldApplyColor = */ true,
        		/* isEditable = */ false, /* EntryContextMenuInfo = */ null,
        		/* thirdIcon = */ null, /* thirdIntent = */ null,
        ...
        promptEntries.add(new ArrayList<Entry>(1));
        promptEntries.add(new ArrayList<Entry>(1));
        promptEntries.get(0).add(addAsNewEntry);
        promptEntries.get(1).add(addToExistEntry);
        mNoContactDetailsCard.initialize(promptEntries, 2, /* isExpanded = */ true,
                /* isAlwaysExpanded = */ true, mExpandingEntryCardViewListener, mScroller);
        mNoContactDetailsCard.setVisibility(View.VISIBLE);
    }
